...
difference() {
  union() {
    ...
    translate([0, 0, KNOB_THICKNESS - SHAFT_INSET])
      cylinder(r=SHAFT_OUTER_RADIUS, h=SHAFT_THICKNESS, $fn=80);
  }
  translate([0, 0, KNOB_THICKNESS - SHAFT_INSET]) difference() {
    ...
    translate([NOTCH_OFFSET, 0, 0])
      cube(
        [3*SHAFT_OUTER_RADIUS, 3*SHAFT_OUTER_RADIUS, NOTCH_THICKNESS],
        anchor=LEFT+BOTTOM
      );
  }
}
